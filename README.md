...
- `evidence assert-live <bundle.json> [--verify-key <pem>]`: fail unless the bundle proves a completed encrypted phone-wallet exchange through request fetch, response receipt, decryption, and offline presentation verification. It does not claim trust/status/over-ask; use the live trace and explicit gates for those.
