...
The bundled plugin includes target binaries for macOS Apple Silicon, macOS
...
the OS blocks the bundled binary, do not pretend the skill can run checks. Say
...
- Export and replay local evidence: turn one `serve --unsafe-debug-artifacts` session directory into a sensitive bundle, verify its hashes and optional ES256 signature, render a redacted replay timeline, use `evidence profile` to summarize trust/status readiness, and use `evidence assert-live` to prove a completed encrypted phone-wallet run after capture. Replay can also report wallet over-disclosure when the captured request has explicit DCQL claim paths; treat that as a redacted auditor signal, not as issuer trust, live status, or legal over-ask proof. Do not use `assert-live` to claim trust/status/over-ask; it proves request fetch, encrypted response receipt, decryption, and offline presentation verification.
- When the user wants to prove an actual phone-wallet demo, follow `docs/PHONE_WALLET_PROOF.md` if the repository checkout is available, or `reference/phone-wallet-proof.md` from plugin-only installs. The sequence is: `serve --host 0.0.0.0 --public-url <reachable-url>/ --unsafe-debug-artifacts <dir>`, scan the QR, export the verified session, then run `evidence verify`, `evidence replay`, and `evidence assert-live`. If a wallet posts twice, unsafe artifacts preserve the first canonical capture and suffix later duplicates such as `direct-post-2.body`. For Bundesdruckerei preprod PID captures in a full checkout, `just bundesdruckerei-wallet-trust-status-proof <bundle.json>` adds the explicit trust/status proof with the current provider material.
