...
| --- | --- |
| What is this blob? Auto-detect and decode anything. | `$AUGENMASS inspect <input>` |
| Same, as JSON for a script or agent. | `$AUGENMASS inspect --json <input>` |
| Read a JWT/JWS (header + payload), no verification. | `$AUGENMASS decode jwt <input>` |
| Read an SD-JWT VC: issuer claims, disclosures, KB-JWT, resolved view. | `$AUGENMASS decode sd-jwt <input>` |
| Read a WRPRC registration certificate (payload-only). | `$AUGENMASS decode regcert <input>` |
| Read an OpenID4VP authorization request / signed JAR. | `$AUGENMASS decode request <input>` |
| Read an OpenID4VCI credential offer (URI or JSON). | `$AUGENMASS decode offer <input>` |
| Read a token status list token. | `$AUGENMASS decode status-list <input>` |
| Read an ISO 18013-5 mdoc (mso_mdoc): namespaces, elements, issuerAuth (COSE alg + x5chain), and the MSO. Decode only. CBOR, hex, or base64. | `$AUGENMASS decode mdoc <input>` |
| Validate a DCQL query: unique ids, credential_sets references, and per-format claim paths (mdoc [namespace, element] vs SD-JWT). Exits non-zero on a blocking error. | `$AUGENMASS validate dcql <input>` |
...
| --- | --- | --- |
| Is this registration body safe to write? (over-ask + format gate) | `$AUGENMASS check <body>` | 1 on over-ask or a blocking format error |
| Same, machine-readable for CI. | `$AUGENMASS check --json <body>` | 1 (as above) |
| Does this request over-ask for a purpose? (defaults: minimal request, event_checkin) | `$AUGENMASS audit --request <minimal\|overask\|FILE\|-> --purpose <id>` | 1 on over-ask |
| Audit a request against a purpose AND the cert's allowed attributes. | `$AUGENMASS audit --request <FILE> --purpose <id> --cert <CERT>` | 1 on over-ask |
| Audit against a non-default credential type. | `$AUGENMASS audit --request <FILE> --purpose <id> --vct <VCT>` | 1 on over-ask |
| List all curated purpose baselines and the legal basis. | `$AUGENMASS baselines` | 0 |
| Show one baseline in detail. | `$AUGENMASS baselines <id>` | 0 |
...
Everything here actually checks cryptography (still offline; no network). `verify presentation` is the full flow for a presentation; `verify request` is the request-side counterpart (it verifies a signed JAR); `trust`, `status`, and `status-list` are the focused checks. Pass `--now <unix-seconds>` for deterministic, reproducible verification.

| Intent (plain English) | Command | Exit on "bad" |
| --- | --- | --- |
| Verify a presentation end to end (issuer sig, KB-JWT, nonce/aud, vct). | `$AUGENMASS verify presentation <p> --nonce <NONCE> --aud <AUD>` | 1 if not verified |
...
| Verify a status-list token and read one index. | `$AUGENMASS verify status-list --token <TOKEN> --key <PEM> --index <N>` | 1 if revoked at that index / error |
| Compute the x509_hash client_id binding from a cert or JAR. | `$AUGENMASS x509-hash <input>` | 0 |
| Check a claimed client_id against the computed binding. | `$AUGENMASS x509-hash <input> --client-id <CLAIMED>` | 1 on mismatch |
...
# Verify a signed request (JAR) and anchor its x5c leaf. Exits 0.
...
| Intent (plain English) | Command |
| --- | --- |
| Generate a proportionate registration body (the age check by default). | `$AUGENMASS generate regbody` |
| Generate an intentionally over-broad body (to demo what `check` catches). | `$AUGENMASS generate regbody --over-broad` |
| Generate a body with your own RP id, purpose, and contacts. | `$AUGENMASS generate regbody --rp <RP> --purpose <TEXT> --support-uri <CONTACT> --privacy-policy <URL>` |
| Build a DCQL query from claim paths. | `$AUGENMASS generate dcql --claim <path>` |
| Build a DCQL query from several claims. | `$AUGENMASS generate dcql --claim <path> --claim <path>` |
...
```sh
$AUGENMASS generate regbody --over-broad \
  | $AUGENMASS check -
...
| Intent (plain English) | Command | Exit on "bad" |
| --- | --- | --- |
| Why is my signed request / JAR rejected? (x5c, client_id gotchas) | `$AUGENMASS doctor <request>` | 1 if findings |
| Same, machine-readable. | `$AUGENMASS doctor --json <request>` | 1 if findings |
...
| --- | --- | --- |
| Run the wallet-interaction debugger (zero-config). | `$AUGENMASS serve` | Throwaway dev cert; runs until Ctrl-C |
| Sign with the real registrar leaf so client_id matches. | `$AUGENMASS serve --key <KEY> --leaf <LEAF>` | Pass both together |
| Enforce issuer trust and live revocation. | `$AUGENMASS serve --trust-anchor <PEM> --status-signer <PEM> --live-status` | `--live-status` needs a trust anchor; real providers often use a separate status signer |
| Make it reachable from a phone wallet. | `$AUGENMASS serve --host 0.0.0.0 --public-url <URL>` | `--public-url` must end in `/` and be reachable from the phone |
| Suppress the live console trace. | `$AUGENMASS serve --quiet` | Still served at `/trace/:id` and `/api/trace/:id` |
| Capture full-fidelity local artifacts. | `$AUGENMASS serve --unsafe-debug-artifacts <DIR>` | UNSAFE, local only, never served over HTTP |

Flags (all optional, each with an env var where noted): `--port` (`PORT`, default 8080), `--host` (`HOST`, default 127.0.0.1), `--public-url` (`PUBLIC_URL`, default `http://127.0.0.1:8080/`, must end in `/`), `--key` (`RP_KEY_PATH`, EC private key PEM), `--leaf` (`RP_LEAF_PATH`, leaf cert PEM), `--purpose` (`PURPOSE`, default `event_checkin`), `--trust-anchor` (`TRUST_ANCHOR_PATH`, PID issuer anchor PEM; when set, the response path rejects issuers that do not chain to it), `--status-signer` (`STATUS_SIGNER_PATH`, token-status-list signer certificate or public key PEM), `--live-status` (`LIVE_STATUS`, default false; resolve the token-status-list over the network and reject revoked/suspended, only effective with a trust anchor), `--quiet` (suppress the live console trace), and `--unsafe-debug-artifacts <DIR>` (`AUGENMASS_UNSAFE_DEBUG_ARTIFACTS`, off by default). This command does not use `--json` and does not exit on its own.
...
| Intent (plain English) | Command | Exit on "bad" |
| --- | --- | --- |
| Export one unsafe debug session directory into a bundle. | `$AUGENMASS evidence export <session-dir> --out <bundle.json>` | 1 if the source manifest or artifacts are invalid |
| Export and sign with ES256. | `$AUGENMASS evidence export <session-dir> --out <bundle.json> --signing-key <pem>` | 1 if export or signing fails |
...
| Prove the captured presentation under explicit issuer trust and status inputs. | `$AUGENMASS evidence prove-trust-status <bundle.json> --trust-anchor <pem> --fetch-status-token --status-key <pem>` | 1 unless the captured presentation verifies with the supplied trust/status material |

`evidence export` writes a JSON bundle with `kind: "augenmass-evidence-bundle"`, `schemaVersion: 1`, `payloadSha256`, `sensitive: true`, raw artifacts as base64url-no-pad entries, a deterministic redacted `replayTrace`, and a machine-readable `caveats` list of handling restrictions. `evidence verify` checks each entry length and SHA-256, regenerates the replay trace, checks the canonical payload hash, and verifies the optional ES256 signature. `evidence replay` performs the same verification first, then prints only the redacted timeline. When the captured request contains explicit DCQL claim paths, replay can also emit wallet over-disclosure analysis as a redacted auditor signal. `evidence assert-live` is stricter: it fails unless the redacted replay contains the completed live-wallet spine (`REQUEST_OBJECT_FETCHED`, `RESPONSE_RECEIVED`, `RESPONSE_DECRYPTED`, `VERIFIED`) with no terminal failure, and reports `walletOverDisclosureAnalyzed` separately when available. It does not claim trust/status/legal over-ask. `evidence prove-trust-status` uses the captured verification context and authorization response, plus explicit `--trust-anchor`, `--status-key`, and exactly one of `--status-token` or `--fetch-status-token`, to prove issuer trust and status without printing wallet claim values. The fetch mode uses the guarded public-address status fetch path.

When the capture contains `direct-post.body`, `session-enc-key.jwk`, `verification-context.json`, and an encrypted response, replay decrypts and verifies the SD-JWT VC offline against the captured nonce, audience, vct, clock, and freshness window. Unsafe artifact writes preserve the first canonical filename on duplicate wallet POSTs and suffix later captures such as `direct-post-2.body`. Replay does not claim trust anchoring or live-status replay.
...
| Intent (plain English) | Command | Notes |
| --- | --- | --- |
| Dry-run a write to the local clone (default target). | `$AUGENMASS register <body>` | Prints what would happen; writes nothing |
| Actually write to the local clone. | `$AUGENMASS register <body> --yes` | Refuses (exit 1) on over-ask; bails on blocking format errors |
| Write past an over-ask warning (deliberate). | `$AUGENMASS register <body> --yes --force` | `--force` requires `--yes` |
| Rehearse a write against the real registrar sandbox. | `$AUGENMASS register <body> --target sandbox --yes` | Off-stage; uses the sandbox env (see below) |
| List registrations for the default relying party (clone). | `$AUGENMASS list` | Defaults `--target clone`, `--rp 2af138a8-...` |
...
$AUGENMASS clone serve --port 8080

# Terminal 2: write a clean body, then read it back.
$AUGENMASS register examples/min.json --yes
$AUGENMASS list
```
