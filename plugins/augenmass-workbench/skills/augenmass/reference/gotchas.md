...
All commands are shown relative to the repo root with bare `augenmass` for readability. Inside the skill, agents should use the resolved `$AUGENMASS` path from the main skill so Claude Code, Codex, and source-installed binaries all work.
